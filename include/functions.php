...
function display_card ($cardnumber,$fourperpage=false,$name_on=false,$rules="") {
	...
	global $headerfontcolor, $headerbgcolor, $mainfontcolor, $mainbgcolor, $selectedfontcolor, $selectedbgcolor, $bordercolor, $printfooter;
			...
			if (isset($set[$cardshown])) { //only if the card actually exists!
				...
				for ($row = 0; $row<5; $row++) {
					...
					for ($column = 0; $column<5; $column++) { //column has to be inner loop due to HTML table
						...
						echo "\n<td align=center>"; // this line replaces previous for background image
				...
				printf("<tr><td align=left colspan=2><b><font size=-1>%s<b></td><td align=center colspan=3><span font-size:14px;> Card Number: %s%'04s </span></td></tr>",(($name_on && $cardshown < count($names))?$names[$cardshown]:''),$setid,$cardshown+1);
				echo "</table></center>";
			}
...
function draws_table() { //table of all numbers drawn
	...
	echo '<table width="100%" border=1 cols=6><tr>';
	if ($draws!=null) {
		$number = count($draws);

		for ($i =0; $i<$number; $i++) {
                  echo '<td align=center width="16.6%">';
                  	echo '<table border="0" cellpadding="0" cellspacing="0" width="50" height="50"><tr>';
					// echo '<td align=center width="16.6%" background="images/ball.gif" style="background-size: 50px 50px; background-repeat: no-repeat;"><b style="font-size: 19px;">'.find_letter($draws[$i]).$draws[$i].'</b></td>';
					echo '<td align=center width="16.6%"><b style="font-size: 32px;">'.find_letter($draws[$i]).$draws[$i].'</b></td>'; // this line replaces the one above to remove bg-image from draws table
                  	echo "</tr></table>";
                  echo '</td>';
			if (($i+1)%6==0) echo "</tr><tr>";
		}
